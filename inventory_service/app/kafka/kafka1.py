...
from aiokafka import AIOKafkaProducer, AIOKafkaConsumer  # Added AIOKafkaConsumer
import json
import logging
from contextlib import asynccontextmanager
from app.settings import BOOTSTRAP_SERVER  # Assuming you have a settings module for configuration
...
async def consume_messages(
    ...
    # process_message: callable,  # A callback function to process each message
    consumer_group_id: str,
    # session: DATABASE_SESSION  # Pass the database session to the consumer
):
    consumer = None  # Ensure consumer is defined even if an error occurs
    try:
        # Initialize Kafka consumer
        consumer = AIOKafkaConsumer(
            topic,
